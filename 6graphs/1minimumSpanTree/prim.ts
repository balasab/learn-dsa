...
interface PrimNeighbor {
    node: number;
    weight: number;
}

interface PrimQueueItem {
    u: number;
    v: number;
    weight: number;
}

interface PrimEdgeResult {
    from: number;
    to: number;
    cost: number;
}
...
class PrimsAlgorithm {
    vertices: number;
    adj: Map<number, PrimNeighbor[]>;

    constructor(vertices: number) {
        this.vertices = vertices;
        this.adj = new Map<number, PrimNeighbor[]>();
    ...
    findMST(): PrimEdgeResult[] {
        const mst: PrimEdgeResult[] = [];
        const visited = new Array(this.vertices).fill(false);

        // Min-Priority Queue (using a simple array sort for demonstration)
        const pq: PrimQueueItem[] = [{ u: -1, v: 0, weight: 0 }];
